...
void displ_0(void){
 ...
 //- Индикация часов -
  ...
  //--- Индикация t ВОЗДУХА ---
  if(Dht){
      temp = set[0][0]; tmpv0 = temp%10; tmpv1 = temp/10;
      sprintf(buff,"[%2u.%u]",tmpv1,tmpv0); // ЗАДАНИЕ T показываем с десятичным знаком
      ...
      if (pvT>1250) sprintf(buff,"t = **.*");
          ...
          temp = pvT; tmpv0 = temp%10; tmpv1 = temp/10;
          sprintf(buff,"t = %2u.%u",tmpv1,tmpv0); // T датчиков показываем с десятичным знаком 
      }
      ...
      for (i=0;i<devices;i++){
        if (i==0) {
            ...
            temp = set[2][0]; tmpv0 = temp%10; tmpv1 = temp/10;
            sprintf(buff,"[%2u.%u]",tmpv1,tmpv0); // ЗАДАНИЕ T показываем с десятичным знаком
            TFT_DrawString(buff,5,pointY-4,1,1,bordWindow,fillWindow);
